...
func TestDefaultConfig_MultipleCalls(t *testing.T) {
	// Test that DefaultConfig generates different session IDs
	cfg1 := DefaultConfig()
	cfg2 := DefaultConfig()
	
	assert.NotEqual(t, cfg1.Session.SessionID, cfg2.Session.SessionID)
	assert.NotEqual(t, cfg1.Session.Key, cfg2.Session.Key)
}

func TestDefaultConfig_SessionKeyGeneration(t *testing.T) {
	cfg := DefaultConfig()
