...
func TestServer_Start_NoTLS(t *testing.T) {
	logger := &mockLogger{}
	cfg := core.DefaultConfig()
	...
	// Start on random port (will fail but tests the code path)
	err := server.Start("127.0.0.1:0")
	// May succeed or fail depending on system
	_ = err
}

func TestServer_Start_TLSEnabled(t *testing.T) {
	logger := &mockLogger{}
	cfg := core.DefaultConfig()
	cfg.Server.TLSEnabled = true
	cfg.Server.TLSCertPath = "/nonexistent/cert.pem"
	cfg.Server.TLSKeyPath = "/nonexistent/key.pem"
